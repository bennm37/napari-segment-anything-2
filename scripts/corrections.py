...
def corrections(video_dir, name="video_segments.pkl"):
    ...
    @viewer.bind_key("cmd+p")
    def save_final_mask(viewer):
        viewer.layers["SAM masks"].save(f"{video_dir}/results/final_mask.tif")
        print(f"Saving SAM masks to {video_dir}/results/final_mask.tif")
        viewer.layers["SAM masks"].status = "Final mask saved"

    run()
    if viewer.layers["SAM masks"].status == "Final mask saved":
        return None, None
    return (
...
if __name__ == "__main__":
    ...
    if positive is None or negative is None:
        print("Final mask was saved. Exiting.")
    else:
        print("No final mask saved. Attempting to correct using SAM.")
        points, labels = create_labels(positive, negative)
        print(f"{points = }, {labels = }")
        np.savetxt(
